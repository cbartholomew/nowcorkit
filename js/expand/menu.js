...
{		
		...
		function toolbar() {
			...
			$('#toolbar').html(html);
			$(function(){ $("#span_menu").buttonset(); });
			$('#' + bar[param].id).attr('checked', true);
			$('#logout').click(function() 	{ window.location = "logout.php"; });
			$('#help').click(function() 	{ that.help_modal(); 	 		  	  });
			$('#menu').click(function() 	{ that.clean();			 		  	  });
			$('#user').click(function() 	{ load_preferences(); 			  	  });	
